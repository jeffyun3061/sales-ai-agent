...
question = "최근 투자 받은 한국 스타트업 알려줘" # 현재는 테스트 용 추후 모든 에이전트 완성 후 LangGraph 완성 시 질문을 상태로 받아 처리하는 방식 등으로 변경 필요
...
class CustomFileSearchTool(BaseTool):
    ...
    description: str = "회사 정보 CSV 데이터를 기반으로 검색합니다."
    csv_path: str = Field(default="company_data.csv")
